...
# 🌟 Strict & safe registration bina kisi optional field ke jhanjhat ke
@admin.register(CompanyClient)
class CompanyClientAdmin(admin.ModelAdmin):
    list_display = ('id', 'company_name')  # ❌ 'created_at' hata diya
    search_fields = ('company_name',)

@admin.register(DataUploadBatch)
class DataUploadBatchAdmin(admin.ModelAdmin):
    list_display = ('id', 'client', 'source_system', 'original_file_name')
...
class CarbonDataRowAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'emission_activity_type', 
        'ghg_scope', 
        'normalized_quantity', 
        'normalized_unit', 
        'calculated_carbon_footprint_kg', 
        'review_status', 
        'is_finalized_and_locked'
    )
